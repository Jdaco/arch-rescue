...
make_pacman_conf() {
    ...
    cat ${script_path}/pacman.conf > ${work_dir}/pacman.conf
}

# Base installation, plus needed packages (airootfs)
make_basefs() {
    setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" init
    setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "haveged intel-ucode memtest86+ mkinitcpio-nfs-utils nbd zsh ansible" install
}
...
make_customize_airootfs() {
    ...
    cp ${verbose} -f ${work_dir}/pacman.conf ${work_dir}/${arch}/airootfs/etc

    # Install ansible-aur
    setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r 'git clone https://github.com/kewlfft/ansible-aur.git /root/.ansible/plugins/modules/aur' run

    # Run the ansible playbook
    setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r 'ansible-playbook -i "localhost," -c local -e "user=root home=/root" /root/ansible-workstation/playbook.yml' run
